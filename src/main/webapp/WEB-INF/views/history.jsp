...
<head>
    ...
    <style>
        ...
        .container {margin-left: 190px; margin-top: 10px; margin-bottom: 10px; padding: 20px; width: 87%; height: 790px; background-color: rgb(255, 255, 255); border-radius: 15px; position: absolute; z-index: 2; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.2);}
        .productStatus {width: 100%; height: 750px; margin: auto; padding: 30px; border-radius: 15px; background-color: #dbedfb; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.2);} 
        .filter {display: flex; justify-content: space-between; align-items: center; padding: 10px; border-bottom: 1px solid #ccc;}
        .filter form {display: flex; align-items: center;}
        .result {display: flex; align-items: center;}
        .inputIcon {display: flex; align-items: center;}
        .select {margin-right: 10px;}
        .refreshBtn {display: flex; align-items: center;}
        strong {margin-left: 10px;}
        button {background: none; border: none;}
        button svg, .refreshBtn svg {fill: #000;}
        input[type="text"] {width: 400px; border: 1px solid #ccc; padding: 5px; margin-left: 10px;}
        .noneDisplay {display: none;}
        table {margin-left: 30px; width: 95%; text-align: center;}
        th, td {padding: 12px; text-align: center; border-bottom: 1px solid #ddd; font-size: 18px; color: dimgrey;}
        .tableContent {height: 49px;}
        .pagination {position: absolute; bottom: 0; left: 50%; transform: translateX(-50%); width: auto; height: 80px; font-weight: bold; font-size: 20px; color: dimgrey;}
        .pagination ul {list-style: none; display: flex; justify-content: center;}
        .pagination li.active {margin-right: 30px;}
        /* side-bar */
                        ...
                        <div class="result">
                            <div class="inputIcon">
                                <select name="condition" id="condition">
                                    ...
                                    <option value="delivery_time" ${strArray[1] eq 'delivery_time' ? 'selected' : ''}>배송 시간</option>
                                    <option value="arrival_time" ${strArray[1] eq 'arrival_time' ? 'selected' : ''}>도착 시간</option>
                                </select> 
                                <button type="submit">
            ...
            <div class="pagination">
                <ul>
                    <c:if test="${intArray[1] ne 1}">
                        <li>
                            <a href="history?pageNum=${intArray[1]-1}&condition=${strArray[1]}&keyword=${strArray[0]}">prev</a>
                        </li>
                    </c:if>
                    <c:forEach var="i" begin="${intArray[1]}" end="${intArray[2]}">
                        <li class="${intArray[0] eq i ? 'active' : ''}">
                            <a href="history?pageNum=${i}&condition=${strArray[1]}&keyword=${strArray[0]}">${i}</a>
                        </li>
                    </c:forEach>
                    <c:if test="${intArray[2] lt intArray[3]}">
                        <li>
                            <a href="history?pageNum=${intArray[2]+1}&condition=${strArray[1]}&keyword=${strArray[0]}">next</a>
                        </li>
                    </c:if>
                </ul>
            </div>
